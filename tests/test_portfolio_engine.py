...
def test_get_stock_data_wraps_a_series_into_a_frame(monkeypatch):
    series = pd.Series([1.0, 2.0], index=pd.bdate_range("2024-01-01", periods=2), name="AAA.KS")
    monkeypatch.setattr(pe.yf, "download", lambda *a, **k: {"Close": series})
    assert isinstance(pe.get_stock_data(["AAA.KS"], "2024-01-01", "2024-01-31"), pd.DataFrame)


# ---------- custom_backtest (얇은 래퍼) ----------

def test_custom_backtest_ignores_missing_tickers_and_matches_engine(prices):
    weights = {"AAA.KS": 0.5, "BBB.KS": 0.5}
    custom = cb.backtest_custom_portfolio(prices, {**weights, "ZZZ.KS": 0.5}, "M", 1_000_000)
...
@pytest.mark.xfail(reason="A8: 데이터에 남는 가중치 합계 0이 ZeroDivisionError가 된다 (단계 1)")
