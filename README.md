...
2. **사용자 정의 백테스트** — 표(티커·종목명·비중%)에 종목별 비중을 입력해 백테스트하고, 성과 차트와 자산 배분을 확인합니다. 저장해 둔 포트폴리오를 불러와 수정할 수 있고(다른 사용자가 만든 것을 불러오면 "(내 복사본)" 이름으로 저장), 종목 추가/제거와 이름을 붙여 저장도 여기서 합니다.
3. **리밸런싱 가이드** — 표에서 종목별 현재 수량과 목표 비중(%)을 입력하면(티커 옆에 종목명 표시, 종목 추가/제거 가능) 종목별 매수/매도 수량, 필요 현금, 예상 거래 비용을 계산합니다.
