...
- **결정 사항 9개는 2026-09-22에 권장안대로 확정**했다(§9). 이 문서는 아직 실행 전이며, 착수는 별도 지시를 기다린다.
...
| A3 | 샤프 지수가 무위험수익률 0 가정인데 화면에 표시가 없다. | `calculate_metrics` | 낮음 | 라벨에 가정 명시, 테스트에 포함 |
...
| F5 | **검증 환경이 배포 환경과 다르다.** 지금까지의 임시 검증은 pandas 3.0.6, Streamlit 1.64.0, yfinance 1.7.0 등 최신 버전으로 했고, 배포는 pandas 2.3.3, Streamlit 1.52.2, yfinance 1.0으로 고정되어 있다. 결측이 없는 데이터에서는 핵심 엔진 결과가 두 pandas 버전에서 완전히 같음을 확인했지만(리밸런싱 횟수 25/9/3, 총수익률·MDD 동일), 결측 처리는 다르다(A2). | 중간 |
...
- [ ] `tests/` 생성, pytest 도입. **외부 Yahoo·GitHub·OAuth는 단위 테스트에서 호출하지 않는다**(고정 fixture, 가짜 서버).
- [ ] 정상 경로 특성화 테스트: 리밸런싱 날짜, 비중 정규화, 매수/매도 분류, 저장소(가짜 서버), 소유권·가져오기, 보유 수량 검증, 로그인 판정 표
- [ ] **경계값·실패 경로 테스트(필수)**: 아래 목록. 현재 동작을 그대로 고정하지 말고, 잘못된 결과를 내는 항목은 "수정 후 기대 동작"을 명세로 적고 단계 1 전까지는 known-failure로 표시한다.
...
- [ ] `AppTest` 스모크: 개발 모드 흐름, 로그인 화면, GitHub만 설정 시 차단
- [ ] 고정 시세 fixture(CSV, 결측이 있는 것 포함)로 네트워크 없이 백테스트 검증
- [ ] GitHub Actions: **배포와 같은 고정 버전으로** 설치 → `py_compile` → pytest. Python 버전 범위는 §5 참고
- **완료 기준**: CI 통과, 이후 단계에서 테스트 없이 코드를 옮기지 않는다.

### 단계 1 — 결함 수정: 입력 검증과 실패 은닉 제거 (규모 중, 동작 변경 PR)

- [ ] 엔진 경계 검증(A8): 날짜·티커·가중치(합계·부호·수치)·가격(0·누락·NaN) 검증을 엔진 함수 입구에 두고 구조화된 예외로 전달. UI는 그 예외를 사용자 메시지로 옮긴다(D4)
- [ ] 가격 조회 계약(A5): **마지막 사용 가능한 거래일 종가**로 기준을 바꾸고(확정 §9-2) 티커별 마지막 유효 가격 + 기준일 반환, 실패·누락은 예외/결과로 전달, `print`·빈 dict 제거, 기준일 불일치 경고. **보유 종목 가격이 없으면 리밸런싱 안내를 만들지 않는다**
- [ ] 날짜 검증(A6): 시작일 > 종료일, 미래 종료일, 종료일 배타성 처리
- [ ] 해당 파일에서 쓰이지 않는 import·함수·`__main__` 정리(C4, C5)를 함께 처리해도 된다
- **완료 기준**: 단계 0의 known-failure 테스트가 모두 통과하고, 가격 누락 재현 시나리오가 잘못된 매매 안내 대신 오류를 낸다.
...
- [ ] `config.py` 신설: `TICKER_NAMES`, `DEFAULT_TARGET_WEIGHTS`, 주기 옵션·기본 기간·자본 상수 (B3, B8)
- [ ] `market_data.py`: 시세 조회를 단일 진입점으로 통합(C1, C2). **캐시 키·TTL을 명시**하고(D1), 결측·빈 결과·상장일 차이·가격 기준일을 결과 객체로 명시
- [ ] 결측·정렬 정책 구현(A2): **공통 관측 구간 기본(확정 §9-3)**, 실제 분석 구간 표시, `fill_method` 명시. 정책은 테스트로 고정
- [ ] 단계 1에서 만든 가격 조회 계약을 `market_data.py`로 이동
- **완료 기준**: 같은 fixture가 pandas 2.3.3과 3.x에서 같은 결과를 낸다(CI 매트릭스).
- **위험**: 결측 정책을 바꾸면 결측이 있는 종목 조합의 결과가 달라진다 → 전후 비교와 릴리스 노트.

### 단계 3 — 정확성 PR (규모 소~중, 수치가 바뀌는 별도 PR)

- [ ] 백테스트를 `backtest.py`로 통합하고 `custom_backtest.py` 삭제(C3). **이 이동 PR과 아래 수치 수정 PR을 분리한다**
- [ ] A1 수정과 성과 지표 정의 변경(위 "A1 수정 사양"), 샤프 가정 표기(A3), `target_weight` 대체 알림(A4)
- [ ] 수동 검산 fixture(100→110→121)를 단위 테스트로 추가
- [ ] 수정 전후 비교표와 릴리스 노트를 PR에 첨부
- **위험**: 화면의 수치가 바뀐다. 기존 fixture 기대값을 갱신할 때 "왜 바뀌었는지"를 PR에 설명한다.

### 단계 4 — 저장소 안정화 (규모 중, **4a 동작 변경 → 4b 파일 이동** 순서)

- [ ] **4a (동작 변경 PR)**: 읽기 검증, `schema_version` 추가, 버전 없는 기존 파일의 v1 마이그레이션(E2), 브랜치 최초 생성 경쟁 422 처리(E6), 동시성·세션 공유 검토(E3), 파일 크기 한계 확인(E4)
- [ ] **4b (동작 불변 PR)**: `storage/backends.py`, `repository.py`, `schema.py`로 분리(E1)
- **완료 기준**: 실제 데이터의 **백업·복구 리허설**을 마쳤다(§5). 4a에서 새 테스트가 통과한 뒤에만 4b를 진행한다.
- **위험**: 저장 형식 변경은 데이터를 잃을 수 있다. 마이그레이션은 읽기 시 메모리에서 먼저 수행하고, 쓰기 시 새 형식으로 저장한다. 시작 전 JSON 백업 다운로드를 안내한다.
