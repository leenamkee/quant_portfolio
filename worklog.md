...
### 종목명 5개를 config.TICKER_NAMES에 추가
- 계기: 다른 프로젝트(`auto_invest`)의 연금계좌 자산배분 리서치 문서에 정리된 5개 후보 포트폴리오를 JSON으로 만들어 이 앱에 가져왔는데(069500, 305080, 132030, 114260, 329200), 전부 `config.TICKER_NAMES`에 없어 `(미등록)`으로만 표시됨. 웹검색으로 각 종목의 실제 KRX 상장 종목명(짧은 표시명, 기존 `config.py` 항목들과 같은 스타일)을 확인해 추가: KODEX 200(069500), TIGER 미국채10년선물(305080), KODEX 골드선물(H)(132030), KODEX 국고채3년(114260), TIGER 리츠부동산인프라(329200).
- **테스트 충돌 발견 및 수정**: `test_tab2_add_and_remove_ticker_rows`가 "등록 안 된 티커" 예시로 `069500`을 쓰고 있었는데, 이번에 그 티커를 등록하면서 이 테스트가 깨지게 됨(더 이상 `(미등록)`이 아니라 "KODEX 200"이 나옴). 테스트를 `999999`(어차피 존재하지 않을 임의의 6자리)로 바꿔 의도(미등록 티커 처리)를 유지.
- 확인: 고정 venv 228통과, pandas 3.x venv(스모크 제외) 204통과. `config.TICKER_NAMES.get()`으로 5개 티커 전부 의도한 한글명이 나오는 것을 직접 확인.
...
- **A3**: `config.py`에 `VOLATILITY_EPSILON`(1e-9), `SHARPE_LABEL`, `format_sharpe()` 추가. 연환산 변동성이 이 값 미만이면(부동소수 오차, 예: 매일 같은 비율로만 성장) 샤프를 `NaN`으로 처리. 앱의 샤프 지수 표시 3곳을 `format_sharpe()`로 통일하고 라벨에 "무위험수익률 0% 가정" 명시.
- 범위 축소(계획서에 기록): `custom_backtest.py` 통합(C3)과 `target_weight` 대체 알림(A4)은 이번 PR에서 하지 않음 — 전자는 이미 얇은 래퍼라 이득이 작아 단계 5로 미루고, 후자는 수치 변경(A1·A3)과 성격이 달라 범위에서 뺌.
- 기존 손계산 테스트 6개(백테스트 결과 시계열 값·길이를 직접 assert하던 것)가 "첫 거래일 제외" 가정으로 쓰여 있어 모두 새 동작에 맞춰 갱신. A1·A3의 xfail 3건을 통과 테스트로 전환하고, 임계값 안팎 동작을 확인하는 테스트 추가 → **211개 통과 + 3개 기대 실패**(E2 2건, E6: 단계 4 대상). pandas 3.0.6 환경에서도 비UI 테스트 동일하게 통과.
- 실제 시세로 임시 복사본에서 탭1·탭2 실행 확인: 오류 없이 지표가 나오고 샤프 라벨이 "샤프 지수 (무위험수익률 0% 가정)"으로 표시됨.
- `app.py`(레거시, 폐기 예정)는 샤프 표시를 그대로 두었다 — 단계 5에서 폐기할 예정이라 "nan" 표시 가능성이 있는 채로 남지만, 실제로 변동성이 정확히 0에 가까운 경우는 드물어 우선순위를 낮게 봄.
- 릴리스 노트에 총수익률·연환산·샤프 변화와 예시 추가.
- 남은 일: 푸시 후 CI 확인, 단계 4(저장소 안정화: 읽기 검증·스키마 버전·브랜치 생성 경쟁)로 진행.
