...
├── portfolio_store.py    # 저장한 포트폴리오 영속화 (GitHub JSON / 로컬 파일)
├── docs/                 # 리서치/설계 문서
...
- 리밸런싱 가이드의 "현재 보유 수량"은 본인의 실제 보유 수량으로 바꿔서 사용하세요.
...
설정하지 않으면 로컬 파일에 저장하며, Cloud에서는 앱이 재시작될 때 사라집니다. 3~4명이 함께 쓰는 방법은 [docs/multi-user-plan.md](docs/multi-user-plan.md)를 참고하세요.
