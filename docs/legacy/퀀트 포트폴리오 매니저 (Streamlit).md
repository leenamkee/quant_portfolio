> **초기 버전 기준 문서, 현재 동작과 다름.** 프로젝트 초기(2026-09 이전)에 작성된 사용 가이드로, 이후 국내 ETF 기본 포트폴리오, DC 연금 위험자산 70% 룰, Google 로그인, 포트폴리오 저장/비교, 리밸런싱 가이드 표 입력 등 많은 부분이 바뀌었다. 현재 기능은 최상위 [README.md](../../README.md)를 참고할 것.
