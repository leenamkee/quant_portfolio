...
def test_missing_local_file_reads_as_empty(local):
    assert ps.load(local) == {"portfolios": []}
...
def test_github_missing_file_reads_as_empty():
    assert github().read() == ({"portfolios": []}, None)
...
@pytest.mark.xfail(reason="E6: 브랜치 최초 생성 경쟁(422)을 일반 오류로 처리한다 (단계 4a)")
...
@pytest.mark.xfail(reason="E2: 손상된 JSON이 StoreError가 아닌 JSONDecodeError로 새어 나간다 (단계 4a)")
...
@pytest.mark.xfail(reason="E2: 읽을 때 형식 검증이 없어 잘못된 구조도 그대로 돌려준다 (단계 4a)")
